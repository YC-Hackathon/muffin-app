 ...
 */
export function createZappGitHubProject(
  repoId: string,
  templateId?: string
): ZappProject {
  ...
  if (templateId === "flutter") {
    ...
    const githubPath = "YC-Hackathon/muffin-flutter-template";

    console.log(
      "createZappGitHubProject Debug - Flutter template, using GitHub path:",
      githubPath
    );

    const url = `https://zapp.run/github/${githubPath}?public=1&cb=${Date.now()}`;
 ...
 */
export function createZappIframe(
  project: ZappProject,
  config: ZappEmbedConfig = {}
): string {
  const embedUrl = generateEmbedUrl(project.url, config);

  return `<iframe src="${embedUrl}" style="width: 100%; height: 100%; border: 0; overflow: hidden;" sandbox="allow-scripts allow-same-origin"></iframe>`;
}
...
export function detectTemplateFromRepo(repoUrl: string): string | null {
  ...
  if (uuidRegex.test(repoUrl)) {
    ...
    console.log(
      "detectTemplateFromRepo Debug - UUID detected, cannot determine template from ID:",
      repoUrl
    );
    return null;
  }
 ...
 * Disabled - all frameworks now use FreestyleDevServer
 */
export function shouldUseZappPreview(templateId: string): boolean {
  return false; // Disabled Zapp.run - use FreestyleDevServer for all frameworks
}
